...
class UNet3D_Midi(nn.Module):
    ...
    def __init__(self, num_modalities, num_classes):
        ...
        super(UNet3D_Midi, self).__init__()
        self.maxpool = nn.MaxPool3d(kernel_size=2, stride=2)
    ...
    def forward(self, x):
        ...
        x = self.down1(x)
        l1_out = torch.clone(x)
        l1_crop_shape = (torch.tensor(l1_out.shape) - torch.tensor([0,0,4,12,12])).tolist()
        l1_out = center_crop(l1_crop_shape[2], l1_crop_shape[3], l1_crop_shape[4], l1_out)
        x = self.maxpool(x)

        x = self.down2(x)
        l2_out = torch.clone(x)
        l2_crop_shape = (torch.tensor(l2_out.shape) - torch.tensor([0,0,6,10,10])).tolist()
        l2_out = center_crop(l2_crop_shape[2], l2_crop_shape[3], l2_crop_shape[4], l2_out)
        x = self.maxpool(x)

        x = self.down3(x)
        l3_out = torch.clone(x)
        l3_crop_shape = (torch.tensor(l3_out.shape) - torch.tensor([0,0,11,13,13])).tolist()
        l3_out = center_crop(l3_crop_shape[2], l3_crop_shape[3], l3_crop_shape[4], l3_out)
        x = self.maxpool(x)
...
class UNet3D_Matchmini(nn.Module):
    ...
    def __init__(self, num_modalities, num_classes):
        ...
        super(UNet3D_Matchmini, self).__init__()
        self.maxpool = nn.MaxPool3d(kernel_size=2, stride=2)
    ...
    def forward(self, x):
        ...
        x = self.down2(x)
        l2_out = torch.clone(x)
        #l2_crop_shape = (torch.tensor(l2_out.shape) - torch.tensor([0,0,8,8,8])).tolist()
        #l2_out = center_crop(l2_crop_shape[2], l2_crop_shape[3], l2_crop_shape[4], l2_out)
        x = self.maxpool(x)
